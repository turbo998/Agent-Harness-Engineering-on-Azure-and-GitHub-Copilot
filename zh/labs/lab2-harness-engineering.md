...
| **身份 / 授权** | AgentCore Identity（OAuth / workload identity） | **Microsoft Entra Agent ID** *（预览）* —— 每 Agent 托管身份、OBO 透传 | — |
...
| **治理 / 注册表** | AgentCore Gateway → Policy / Registry | **Azure API Center**（API + MCP 注册）+ **Entra Agent Registry** *（预览）* | repo 结构 / 命名约束 |
...
graph TB
    ...
    Repo ==>|同一套原语，上移一层| AWS
    Repo ==>|同一套原语，上移一层| AZ
    subgraph Managed["托管运行时 harness"]
        direction LR
        AWS["AWS Bedrock AgentCore<br/>Runtime · Memory · Gateway<br/>Identity · Observability · Governance"]
        AZ["Microsoft Foundry Agent Service<br/>Hosted agents · Cosmos 状态 · MCP 工具<br/>Entra Agent ID · Tracing/OTel · API Center"]
    end
```
...
> ⚠️ **中国区合规闸门（别盲目承诺）：** 上表的 Azure 映射描述的是 **Azure Global**。Microsoft Foundry Agent Service、Entra Agent ID，以及表中若干工具（memory、Toolbox）处于**预览**状态，且在 **Azure 中国（世纪互联）可用性受限**。在做任何中国区落地承诺之前，必须逐项核实区域可用性与数据驻留边界。
