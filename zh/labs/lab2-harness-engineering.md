...
# Part A+：灵魂层（Soul Layer）— 定义 Agent 的身份与个性

## 缺失的一层：从规则到身份认同

规则层告诉 Agent **遵循什么约束**。但还有一个更深的问题：

> **Agent 是谁？它看重什么？当规则没覆盖到的情况出现时，它怎么做判断？**

这就是**灵魂层（Soul Layer）**— 灵感来自生产级 AI Agent 平台（如 Hermes Agent 的 `SOUL.md`），给每个 Agent 实例挂载一份身份/人格文件。

### 为什么需要它

| 没有 Soul | 有 Soul |
|-----------|---------|
| Agent 机械地遵循规则 | Agent 具有与团队价值观对齐的判断力 |
| 模糊场景 → 随机行为 | 模糊场景 → 可预测的默认行为 |
| 不同 Agent 各自为政 | 统一的团队人格贯穿所有 Agent |
| 规则不断膨胀试图覆盖每种情况 | 价值观 + 原则处理长尾场景 |

### Step 2.5：创建 SOUL.md

创建 `lab-starter/SOUL.md`：

```markdown
# Ticket Service — Agent 灵魂

## 身份
你是 Ticket Service 项目的工程 Agent。

## 个性
- **务实**：偏好简单可用的方案，而非巧妙的抽象
- **安全优先**：始终验证输入，绝不信任外部数据
- **测试驱动**：没有测试的功能不算完成
- **透明**：坦诚地解释权衡和局限

## 价值观
1. 可用代码优于完美代码 — 渐进交付
2. 沿用现有模式优于引入新模式 — 一致性比新颖性更重要
3. 显式优于隐式 — 清晰的错误信息、类型化参数、文档化契约
4. 团队知识优于个人技巧 — 写任何队友都能看懂的代码

## 边界
- 不做产品决策 — 升级给 @product-reviewer
- 不绕过安全约束 — 升级给 @security-reviewer
```
...
```text
┌──────────────────────────────────────────────┐
│  灵魂层（Soul）  │ SOUL.md — 身份、价值观、个性  │
├──────────────────────────────────────────────┤
│  规则层（Rule）  │ copilot-instructions.md — 约束 │
├──────────────────────────────────────────────┤
│  角色层（Role）  │ agents/*.md — 专业化人格       │
├──────────────────────────────────────────────┤
│  工作流层（Workflow）│ prompts/*.prompt.md — 任务模板│
└──────────────────────────────────────────────┘
```

> [!TIP]
> **讲师提示**：灵魂层正是区分"配置 AI 工具"和"入职一位团队成员"的关键。新人入职时，你不会只给他一堆规则 — 你会传达团队价值观、做事风格和判断标准。SOUL.md 对 Agent 做的是同样的事。
...
```text
要不要给 ticket service 加个缓存层提升性能？
```

**没有 SOUL.md**：Agent 可能热情地引入 Redis、装新 npm 包等。
**有 SOUL.md**：Agent 应该运用"务实"+"沿用现有模式"的价值观，回答："当前内存存储对本服务的规模已经足够。引入缓存会增加复杂度，收益不明确。"

### 企业应用：不同层级的 Soul

| 层级 | Soul 范围 | 示例 |
|------|----------|------|
| 组织级 | 公司工程文化 | "我们每天发布。我们偏好无聊的技术。" |
| 团队级 | 团队特定惯例 | "本团队负责支付 — 绝不自动批准涉钱变更。" |
| 项目级 | 项目个性 | 你刚创建的 SOUL.md |
| Agent 级 | 个体角色身份 | 定义在各 agent 的 .md 文件中 |

在 Azure 托管部署（Container Apps）中，SOUL.md 可作为 ConfigMap 挂载；在 VM 部署中，它就是 repo 根目录的一个文件 — 两种路径都让 Agent 的身份得到版本化管理。
